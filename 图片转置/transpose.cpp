...
//VAR
const char *READ_PATH="tp1.ppm";
const char *WRITE_PATH="tp2.ppm";
...
void handle()
{
	...
	unsigned int width=inPPM.height;
	unsigned int height=inPPM.width;
	outPPM.width=width;
	outPPM.height=height;
	outPPM.maxColor=inPPM.maxColor;
	outPPM.data.resize(height*width);
	
	for(int y=0;y<height;y++)
		for(int x=0;x<width;x++)
		{
			outPPM.data[y*width+x]=inPPM.data[x*height+y];
		}
