...
//VAR
const char *READ_PATH="invert1.ppm";
const char *WRITE_PATH="invert2.ppm";
...
RGBPixel invert(const RGBPixel &source ,int maxcolor)
{
	RGBPixel p;
	p.r=maxcolor-source.r;
	p.g=maxcolor-source.g;
	p.b=maxcolor-source.b;
	return p;
}
...
void handle()
{
	...
	for(int y=0;y<height;y++)
		for(int x=0;x<width;x++)
		{
			outPPM.data.push_back(invert(inPPM.data[y*width+x],inPPM.maxColor));
		}
